...
int desired_temperature, heater_actif, dutycycle_cooling;
float moy_temperature, moy_humidity;
int DELTA_temp = 1, pwm_stop = 0;

int cooling_100=100, cooling_45 = 45, heater_60 = 60;
...
void asservissement(int desired_temperature)
{
	  ...
	  } else if(heater_actif == 0) // If we receive a 0
	  {
		  HAL_Delay(50);
		  heater_stop();
		  //__nop();
